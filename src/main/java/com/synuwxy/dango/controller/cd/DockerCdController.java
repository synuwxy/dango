...
public class DockerCdController {
    ...
    @PostMapping("/deploy")
    public ResultObject<?> deploy(@Validated @RequestBody DockerDeployParam dockerDeployParam) {
        dockerCdService.deploy(dockerDeployParam);
        return ResultObject.success();
    }

    @PostMapping("/slideDeploy")
    public ResultObject<?> slideDeploy(@Validated @RequestBody DockerDeployParam dockerDeployParam) {
